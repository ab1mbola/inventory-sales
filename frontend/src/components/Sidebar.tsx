...
const links = [
  ...
  { to: '/credit', label: 'Credit Tracking', icon: Wallet },
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-background/80 backdrop-blur-md border-b border-border flex items-center justify-between px-6 z-50">
        <h1 className="font-sans text-xl font-black tracking-tight uppercase">Mnemos</h1>
        <button 
      ...
      <motion.aside 
        ...
        className={`fixed lg:sticky top-0 left-0 z-[70] h-screen bg-surface border-r border-border flex flex-col transform ${
      ...
      >
        ...
        <div className={`px-6 py-10 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
          {!isCollapsed && (
            ...
            >
              <h1 className="font-sans text-2xl font-black tracking-tighter leading-none">MNEMOS</h1>
          ...
          {isCollapsed && <div className="font-sans text-2xl font-black text-accent">M</div>}
        ...
        <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto no-scrollbar">
          {links.map(({ to, label, icon: Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-4 px-4 py-3.5 text-[10px] uppercase tracking-[0.2em] transition-all relative group rounded-xl ${
                  isActive
                    ? 'text-accent bg-accent-soft font-bold active'
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  {isCollapsed && (
                    <div className="absolute left-full ml-4 px-4 py-2 bg-primary text-white text-[9px] tracking-[0.2em] uppercase rounded-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 whitespace-nowrap shadow-xl">
                      {label}
                    </div>
                  )}
                </>
        ...
        <div className="p-6 border-t border-border bg-surface/50">
            <div className={`flex items-center gap-4 ${isCollapsed ? 'justify-center' : ''}`}>
               <motion.div 
                 ...
                 className="w-10 h-10 border border-primary rounded-full flex items-center justify-center text-[10px] font-bold bg-white cursor-pointer hover:border-accent hover:text-accent transition-colors"
               >
