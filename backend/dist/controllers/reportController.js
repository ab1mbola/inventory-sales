...
exports.getReportStats = void 0;
const prisma_1 = require("../utils/prisma");
const date_fns_1 = require("date-fns");
const getReportStats = async (req, res) => {
    try {
        const now = new Date();
        const sixMonthsAgo = (0, date_fns_1.startOfMonth)((0, date_fns_1.subMonths)(now, 5));
        ...
        for (let i = 5; i >= 0; i--) {
            const monthStr = (0, date_fns_1.format)((0, date_fns_1.subMonths)(now, i), 'MMM yyyy');
            monthlyDataMap.set(monthStr, { month: monthStr, revenue: 0, profit: 0 });
        }
        sales.forEach((sale) => {
            const monthStr = (0, date_fns_1.format)(sale.createdAt, 'MMM yyyy');
        ...
        // 2. Sales by Category
        const categorySalesRaw = await prisma_1.prisma.saleItem.findMany({
            include: {
                product: {
        ...
        const paymentDistributionRaw = await prisma_1.prisma.sale.groupBy({
            by: ['paymentMethod'],
            _count: { id: true },
